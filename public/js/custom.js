...
var getSlides = function getSlides(ele) {
  return Array.from((ele || document).querySelectorAll('.glide__slide'));
};

var getTotalSlides = function getTotalSlides(ele) {
  return getSlides(ele).length;
};
/**
...
var parseSliderOptions = function parseSliderOptions(data) {
...
var addSlidesIndex = function addSlidesIndex(ele) {
  return getSlides(ele).forEach(function (slide, index) {
    return slide.setAttribute('data-glide-index', index);
  });
};
/**
...
var getGliderByComponentName = function getGliderByComponentName(componentName) {
  return window.glideSliders[componentName];
};
/**
...
var getWrapperByComponentName = function getWrapperByComponentName(componentName) {
  return getGliderByComponentName(componentName).selector.parentElement;
};
/**
...
var updateProgressBar = function updateProgressBar(_ref) {
  var componentName = _ref.componentName;
  var progressBar = getWrapperByComponentName(componentName).querySelector('.glide__scrollbar-progress');
  var currentIndex = getGliderByComponentName(componentName).index + 1;
  var totalSlide = getTotalSlides(getWrapperByComponentName(componentName));
  progressBar.style.width = "".concat(currentIndex / totalSlide * 100, "%");
};
/**
...
var addProgressBar = function addProgressBar(gd, _ref2) {
  var componentName = _ref2.componentName,
      progressBarEnabled = _ref2.progressBarEnabled;

  if (progressBarEnabled) {
    gd.on(['move.after'], function () {
      updateProgressBar({
...
var createBulletItem = function createBulletItem(index) {
  var bulletEle = document.createElement('button');
  bulletEle.setAttribute('data-glide-dir', "=".concat(index));
...
var addBullets = function addBullets(ele, _ref3) {
  var bullets = _ref3.bullets;
  if (!bullets) return;
  var bulletsWrapper = document.createElement('div');
  bulletsWrapper.classList.add('glide__bullets');
  bulletsWrapper.setAttribute('data-glide-el', 'controls[nav]');
  Array.from(ele.querySelectorAll('.glide__slide')).forEach(function (item, index) {
    return bulletsWrapper.appendChild(createBulletItem(index));
  });
...
var addArrows = function addArrows(ele) {
  var arrowsWrapper = document.createElement('div');
  arrowsWrapper.classList.add('glide__arrows');
  arrowsWrapper.setAttribute('data-glide-el', 'controls'); // ArrowLeft

  var arrowsLeft = document.createElement('button');
  ...
  arrowsWrapper.appendChild(arrowsLeft); // ArrowLeft
...
var initGlide = function initGlide(ele, index) {
  addSlidesIndex(ele);
  var options = parseSliderOptions(ele.dataset.slider);
  ...
  var gd = new _glidejs_glide__WEBPACK_IMPORTED_MODULE_0__["default"](ele, options);
  var divCount = ele.querySelector('.glide__arrow-count');

  if (divCount) {
    var maxCount = ele.querySelectorAll('.glide__slide').length;
    gd.on(['mount.after', 'run'], function () {
      divCount.innerHTML = "".concat(gd.index + 1, "/").concat(maxCount);
    });
  }

  gd.mount();
  addProgressBar(gd, options);
  var componentName = options.componentName || index;
  return _defineProperty({}, componentName, gd);
};
/**
...
var initSliders = function initSliders() {
  ...
  window.glideSliders = Array.from(document.querySelectorAll('.glide')).reduce(function (acc, ele, index) {
    var gd = initGlide(ele, index);
    return _objectSpread(_objectSpread({}, acc), gd);
  }, {});
};
/**
